...
def key_sentence_page(api: ApiRequest, is_lite: bool = False):
    ...
    with st.container():
        genre = st.radio(
            "样例:",
            ["不使用样例", "样例1", "样例2"],
            captions=["", "“光纤之父”高锟 2009年...", "THE GIANTS RETURN By ROBERT ABERNATHY Earth..."],
        ...
        if genre == '样例1':
            ...
            st.session_state[
                "options"] = "A#幼年的高锟热衷化学实验，后来又迷恋无线电，这段经历表现出的特质对他后来进行光纤通信研究具有重要的作用。B#高锟先生为人谦虚，对人和蔼，关心家人，用实际行动支持学生自由发表言论，表现了一位科学家的高尚美德。C#文章引用高锟的妻子黄美芸和网友的话，突出了高锟在光纤通信科研领域的重大贡献，表达了对高锟的崇敬之情。D#这篇传记记述了传主高锟人生中的一些典型事件， 通过正面和侧面描写来表现传主，生动形象，真实感人。"
        elif genre == '样例2':
            ...
            """
            st.session_state["options"] = """A#People from Earth would have colonized the Procyon system.B#Captain Llud would have become a hero.C#The other two Quest ships would have been launched.D#Humanity would have died out.
            """
        else:
        ...
        with option_c:
            options = st.text_area("选项", placeholder="请输入选项...(请使用#分隔选项)", height=250, key="options")
    ...
    def submit():
        ...
        if CUR_LLM.__contains__('ncr') or CUR_LLM.__contains__('cclue'):
            ...
            language = 'en'
        render_context = render_key_sentence(language=language, question=question, options=options, context=passage, max_word_count=2000)
